...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec.exec('python -m pip install pip twine');
            yield exec.exec('python -m pip install -r requirements.txt');
            // Package
            yield exec.exec(`python setup.py sdist --dist-dir ${runner.temp}`);
            // Create necessary config for twine
            writePypirc();
            // Upload
            yield exec.exec(`twine upload -r nimport --config-file "${PypircPath}" ${runner.temp}/* --skip-existing`);
        }
        catch (error) {
